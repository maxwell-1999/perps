...
export { Button, IconButton, ButtonGroup, type ButtonProps, type IconButtonProps } from './Button'

// import { CircularProgress } from '@mui/material';
const CircularProgress = () => {
  return <div>Loading...</div>
}
export const buttonAnimation = ' hover:translate-y-[-2.5px] active:translate-y-1 '
...
const CustomButton: React.FC<IButton> = ({
  ...
  return (
    ...
    >
      {isLoading ? <CircularProgress className="!w-[15px] !h-[15px] mx-[10px]" color="inherit" /> : children}
    </button>
  )
}
export const BufferButton: React.FC<IButton> = ({
  ...
  return (
    <button
      ...
      className={`${buttonAnimation} !pointer-events-auto flex justify-center items-center transition-all duration-300 w-full rounded-[5px] text-f16 disabled:bg-cross-bg disabled:text-3 disabled:cursor-not-allowed h-[36px]  ${className}`}
    >
      {isLoading ? <CircularProgress className="!w-[20px] !h-[20px]" color="inherit" /> : children}
    </button>
  )
}
...
export const RedBtn: React.FC<IButton> = ({ isLoading = false, children, isDisabled, className, onClick }) => {
  return (
    <CustomButton
      isLoading={isLoading}
...
export const BlackBtn: React.FC<IButton> = ({ isLoading = false, children, isDisabled, title, className, onClick }) => {
  return (
    <CustomButton
      isLoading={isLoading}
      isDisabled={isDisabled}
...
export const GreyBtn: React.FC<IButton> = ({ isLoading = false, children, isDisabled, className, onClick }) => {
  return (
    <CustomButton
      isLoading={isLoading}
      isDisabled={isDisabled}
